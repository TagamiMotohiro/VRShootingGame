...
public class VRPlayerWork : MonoBehaviour
{
    ...
    [SerializeField] bool stert = false;
    ...
    private void HandShakeControler()
    {
        ...
        if (Mathf.Abs(touchVelocityL.y) > Mathf.Abs(touchVelocityR.y))
        {
        ...
        Quaternion ort, Vector3 handShakeVel, Vector3 handShakeAcc)
    {
        ...
        bool isWalk = DetectHandShakeWalk(Mathf.Abs(handShakeVel.y)) || motionInertia;
        if (isWalk)
        {
            if (!motionInertia)
                SetMotionInertia();
            ...
            bool isRun = DetectHandShakeRun(Mathf.Abs(handShakeVel.y));
            if (isRun)
